...
class AopClient {
	...
	protected function logCommunicationError($apiName, $requestUrl, $errorCode, $responseTxt) {
		...
		//$logger->log($logData);
		debuglog('logCommunicationError:'.iconv('GBK','UTF-8',$logData[8]));
	}
	...
	*/
	...
	public function execute($request, $authToken = null, $appInfoAuthtoken = null,$notchecksign=null) {
		...
		$r = iconv('GBK','UTF-8' , $resp);


		$signData = null;

		if ("json" == $this->format) {
			debuglog('____________________go0');
			$respObject = json_decode($r);
			debuglog('____________________go1'.$respObject);
			if (null !== $respObject) {
			debuglog('____________________go1');
				$respWellFormed = true;
		...
		if (false === $respWellFormed) {
			debuglog('____________________go2');
			$this->logCommunicationError($sysParams["method"], $requestUrl, "HTTP_RESPONSE_NOT_WELL_FORMED", $resp);
			return false;
		}
		debuglog('____________________go3');
		if($notchecksign!=null){

			$respObject=$signData->signSourceData;
		}else {
	...
	public function checkResponseSign($request, $signData, $resp, $respObject) {
		...
		if (!$this->checkEmpty($this->alipayPublicKey) || !$this->checkEmpty($this->alipayrsaPublicKey)) {
			...
			if (!$this->checkEmpty($responseSubCode) || ($this->checkEmpty($responseSubCode) && !$this->checkEmpty($signData->sign))) {
				...
				//debuglog('AOP验证签名checkResult'.$checkResult);
				if (!$checkResult) {
					//debuglog('AOP验证签名checkResult为空 进入空值判断');
					if (strpos($signData->signSourceData, "\\/") > 0) {
						//debuglog('AOP验证签名checkResult为空 进入if签名strpos方法');
						$signData->signSourceData = str_replace("\\/", "/", $signData->signSourceData);

						$checkResult = $this->verify($signData->signSourceData, $signData->sign, $this->alipayPublicKey, $this->signType);
						//debuglog('AOP验证签名checkResult为空 再次验证checkResult：'.$checkResult);
						if (!$checkResult) {
							//throw new Exception("check sign Fail! [sign=" . $signData->sign . ", signSourceData=" . $signData->signSourceData . "]");
							$errormsj="check sign Fail -1! [sign=" . $signData->sign . ", signSourceData=" . $signData->signSourceData . "]";
							debuglog($errormsj);
						}

					} else {
						debuglog('AOP验证签名checkResult为空 进入if签名strpos方法');
						$signSourceData=iconv('GBK','UTF-8',$signData->signSourceData);
						//throw new Exception("check sign Fail! [sign=" . $signData->sign . ", signSourceData=" . $signData->signSourceData . "]");
						//throw new Exception("check sign Fail! [sign=" . $signData->sign . ", signSourceData=" . $signSourceData . "]");
						$errormsj="check sign Fail -2! [sign=" . $signData->sign . ", signSourceData=" . $signSourceData . "]";
						debuglog($errormsj);
					}
